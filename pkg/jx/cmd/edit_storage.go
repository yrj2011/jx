...
` + storageSupportDescription + opts.SeeAlsoText("jx step stash", "jx get storage"))
	...
	editStorageExample = templates.Examples(`
		...
		jx edit storage -c logs --git-url http://github.com/myorg/mylogs.git'

		# Configure the git URL and branch of where to store logs
		jx edit storage -c logs --git-url http://github.com/myorg/mylogs.git' --git-branch cheese

		# Configure the git URL of where all storage goes to by default unless a specific classifier has a config
		jx edit storage -c default --git-url http://github.com/myorg/mylogs.git'
